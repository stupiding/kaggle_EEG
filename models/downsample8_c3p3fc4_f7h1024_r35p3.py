...
def build_model():
    ...
    conv1 = Conv2DLayer(incoming = pool0, num_filters = 8, filter_size = (1, 7),
    ...
    conv2 = Conv2DLayer(incoming = drop1, num_filters = 16, filter_size = (1, 7),
                        stride = 1, border_mode = 'same',
                        W = nn.init.Normal(std = std),
                        nonlinearity = None)
    ...
    conv3 = Conv2DLayer(incoming = drop2, num_filters = 16, filter_size = (1, 7),
                        stride = 1, border_mode = 'same',
                        W = nn.init.Normal(std = std),
                        nonlinearity = None)
    ...
    fc4 = nn.layers.DenseLayer(incoming = drop3, num_units = 512,
                                 W = nn.init.Normal(std = std),
                                 nonlinearity = None)
    ...
    fc5 = nn.layers.DenseLayer(incoming = drop4, num_units = 512,
                                 W = nn.init.Normal(std = std),
                                 nonlinearity = None)
    ...
    fc6 = nn.layers.DenseLayer(incoming = drop5, num_units = 1024,
                                 W = nn.init.Normal(std = std),
                                 nonlinearity = None)

    bn6 = BatchNormLayer(incoming = fc6, epsilon = 0.0000000001,
                         nonlinearity = nn.nonlinearities.leaky_rectify)
    print 'bn6', nn.layers.get_output_shape(bn6)

    drop6 = nn.layers.DropoutLayer(incoming = bn6, p = p)

    l_out = nn.layers.DenseLayer(incoming = drop6, num_units = num_events,
                                 W = nn.init.Normal(std = std),
